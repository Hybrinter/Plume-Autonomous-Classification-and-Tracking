...
class LqrController:
    ...
    K: object  # np.ndarray[float64, (2,4)]
    ...
    def from_config(cfg: ControllerConfig) -> LqrController:
        ...
        Q = np.diag(np.array(cfg.lqr_Q_diag, dtype=np.float64))
        R = np.diag(np.array(cfg.lqr_R_diag, dtype=np.float64))
        try:
            P = scipy.linalg.solve_discrete_are(A, B, Q, R)
            K = np.linalg.inv(R + B.T @ P @ B) @ (B.T @ P @ A)
        except (ValueError, np.linalg.LinAlgError):
            # Fallback to proportional control if DARE fails
            K = np.zeros((2, 4), dtype=np.float64)
...
def compute_control(
    ...
    state_error: object,  # np.ndarray[float64, (4,)] -- x_current - x_desired
) -> object:  # np.ndarray[float64, (2,)] -- [pan_cmd_deg_s, tilt_cmd_deg_s]
