"""Unit tests for pact.controller.safety — confidence gate, area gate, deadband, rate limit.
...
def test_deadband_above_max_returns_error() -> None:
    ...
    assert isinstance(result, Err), f"Expected Err, got Ok({result.value if hasattr(result, 'value') else result})"
