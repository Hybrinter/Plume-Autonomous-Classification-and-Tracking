...
import torch
from tools.ml_models.arch.classifier import build_classifier
from tools.ml_models.arch.compact import PactNet
from tools.ml_models.arch.unet import UNet, build_segmentor


def test_unet_preserves_spatial_size() -> None:
    """U-Net maps (N, 3, H, W) to (N, 1, H, W) logits for more than one size."""
...
def test_classifier_emits_one_logit() -> None:
    """Default pactnet maps (N, 3, H, W) to (N, 1)."""
