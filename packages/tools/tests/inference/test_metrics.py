"""Closed-form checks for the torch design-metric library."""
...
def test_compute_iou() -> None:
    ...
    empty = torch.zeros((4, 4), dtype=torch.float32)
...
def test_roc_auc_perfect_and_empty() -> None:
    ...
    assert roc_auc(torch.tensor([]), torch.tensor([])) == 0.0
    assert roc_auc(torch.tensor([1.0, 2.0]), torch.tensor([1.0, 1.0])) == 0.0
...
def test_classifier_metrics_empty_and_perfect() -> None:
    ...
    empty = classifier_metrics(torch.zeros((0, 1)), torch.zeros((0, 1)))
    assert empty.n == 0
...
def test_segmentor_metrics_reports_blob_gate() -> None:
    ...
    logits = torch.full((2, 1, 4, 4), 6.0, dtype=torch.float32)
    masks = torch.ones((2, 1, 4, 4), dtype=torch.float32)
    ...
    empty = segmentor_metrics(torch.zeros((0, 1, 4, 4)), torch.zeros((0, 1, 4, 4)))
    assert empty.n == 0
...
def test_binary_cross_entropy_with_logits_zero_on_empty() -> None:
    ...
    assert binary_cross_entropy_with_logits(torch.zeros((0,)), torch.zeros((0,))) == 0.0
    loss = binary_cross_entropy_with_logits(torch.tensor([8.0]), torch.tensor([1.0]))
    assert loss < 0.01


def test_sigmoid_maps_zero_to_half() -> None:
    """sigmoid(0) is 0.5."""
    assert float(sigmoid(torch.tensor([0.0]))[0].item()) == pytest.approx(0.5)
