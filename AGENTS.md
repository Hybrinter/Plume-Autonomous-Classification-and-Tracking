...
This is a Python 3.14+ `uv` workspace (virtual root; members under `packages/` plus `analysis/`). Dependencies are
...
- Standard commands are documented in `README.md` ("Run the gates") and `.github/workflows/ci.yml`.
  The full gate set is: `uv run ruff check packages scripts analysis`, `uv run ruff format --check packages
  scripts analysis`, `uv run mypy packages scripts analysis`, `uv run lint-imports`, `uv run python
...
- Design studies live under `analysis/` (`uv run python -m analysis.studies.<study> ...`). They are
  not on pytest; ruff and mypy cover them. Do not confuse them with `tools.analysis`.
