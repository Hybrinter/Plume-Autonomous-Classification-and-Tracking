...
Contains:
  ...
  - FULL_FRAME_HIT_RATE: summary field for a flight run.
  - STUDY_DICE_METRIC: val Dice field for a study or chip run.
  - FrontierPoint: one run reduced to metric and cost.
  - frontier_points: reduce run summaries to comparable points.
  - has_full_frame_hit_rate: whether a summary carries the flight metric.
  - partition_catalog: flight runs, then study and chip runs.
...
FULL_FRAME_HIT_RATE = "full_frame_hit_rate"
STUDY_DICE_METRIC = "mean_dice"
...
def _split_score(row: dict[str, object], metric: str, split: str) -> float:
    ...
    Notes:
        ...
        ``full_frame_hit_rate`` is also read from the unprefixed summary field.
    """
    score = _scalar(row, f"{split}_{metric}")
    if not math.isnan(score):
        return score
    if metric == FULL_FRAME_HIT_RATE:
        direct = _scalar(row, FULL_FRAME_HIT_RATE)
...
def has_full_frame_hit_rate(row: dict[str, object]) -> bool:
    """Return True when the summary carries a numeric full-frame hit rate."""
    return not math.isnan(_scalar(row, FULL_FRAME_HIT_RATE, f"val_{FULL_FRAME_HIT_RATE}"))


def partition_catalog(runs: tuple[Path, ...]) -> tuple[tuple[Path, ...], tuple[Path, ...]]:
    """Split runs into flight runs and study or chip runs.

    Args:
        runs: Run directories.

    Returns:
        tuple[tuple[Path, ...], tuple[Path, ...]]: Flight runs that carry
        ``full_frame_hit_rate``, then the remaining study and chip runs.
    """
    flight: list[Path] = []
    study: list[Path] = []
    for path in runs:
        if has_full_frame_hit_rate(load_summary(path)):
            flight.append(path)
        else:
            study.append(path)
    return tuple(flight), tuple(study)


def flight_pareto(runs: tuple[Path, ...]) -> tuple[FrontierPoint, ...]:
    """Return the frontier of parameter count against full-frame hit rate.

    Args:
        runs: Run directories. Runs without ``full_frame_hit_rate`` are omitted.

    Returns:
        tuple[FrontierPoint, ...]: Non-dominated flight runs, cheapest first.
        Cost is ``n_params``. Score is the full-frame hit rate.
    """
    flight, _study = partition_catalog(runs)
...
def study_pareto(runs: tuple[Path, ...]) -> tuple[FrontierPoint, ...]:
    """Return the frontier of parameter count against validation Dice.

    Args:
        runs: Run directories. Runs that carry ``full_frame_hit_rate`` are
            omitted. The remaining study and chip runs are scored on val Dice.

    Returns:
        tuple[FrontierPoint, ...]: Non-dominated study and chip runs, cheapest
        first. Cost is ``n_params``. Score is ``val_mean_dice``.
    """
    _flight, study = partition_catalog(runs)
    points = frontier_points(
        study,
        STUDY_DICE_METRIC,
        cost_key="n_params",
