...
def _payload_signals() -> list[Signal]:
    ...
    return [
        ...
        _num(
            "payload.seen_vision",
            "payload",
            "Vision measurement accepted",
            "bool",
            _bool(lambda ctx: ctx.payload_state.seen_vision),
        ),
        _num(
            ...
            "deg/s",
            lambda ctx: float(ctx.payload_state.r_az_deg_s),
        ),
        _num(
            ...
            "deg/s",
            lambda ctx: float(ctx.payload_state.r_el_deg_s),
        ),
        _num(
        ...
        _num(
            ...
            "deg",
            lambda ctx: float(ctx.payload_state.kalman.az.x[0]),
        ),
        _num(
            ...
            "deg",
            lambda ctx: float(ctx.payload_state.kalman.el.x[0]),
        ),
        _num(
            ...
            "Kalman azimuth rate",
            "deg/s",
            lambda ctx: float(ctx.payload_state.kalman.az.x[3]),
        ),
        _num(
            ...
            "deg/s",
            lambda ctx: float(ctx.payload_state.kalman.el.x[3]),
        ),
        _num(
            ...
            "deg^2",
            lambda ctx: float(
                np.trace(ctx.payload_state.kalman.az.p) + np.trace(ctx.payload_state.kalman.el.p)
            ),
        ),
        _num(
            "payload.servo_integral_az",
            "payload",
            "Rate-PI azimuth integral",
            "rad",
            lambda ctx: float(ctx.payload_state.servo.integral_az),
        ),
        _num(
...
def _enrichment_signals() -> list[Signal]:
    ...
    signals: list[Signal] = [
        _num(
            ...
            "deg^2",
            lambda ctx: float(ctx.payload_state.kalman.az.p[0, 0]),
        ),
        _num(
            "payload.kalman_p11",
            "payload",
            ...
            "deg^2",
            lambda ctx: float(ctx.payload_state.kalman.el.p[0, 0]),
        ),
        _num(
            "payload.kalman_p22",
            "payload",
            ...
            "deg^2/s^2",
            lambda ctx: float(ctx.payload_state.kalman.az.p[3, 3]),
        ),
        _num(
            "payload.kalman_p33",
            "payload",
            ...
            "deg^2/s^2",
            lambda ctx: float(ctx.payload_state.kalman.el.p[3, 3]),
        ),
        _num(
