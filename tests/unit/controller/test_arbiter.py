"""Unit tests for pact.controller.arbiter — GimbalArbiter state machine.
...
def test_valid_transitions_exhaustive() -> None:
    ...
    """
    # Expected transitions per §5.3 of the arch spec
    expected_transitions: dict[GimbalState, frozenset[GimbalState]] = {
        ...
        GimbalState.ACQUIRING: frozenset({GimbalState.TRACKING, GimbalState.IDLE, GimbalState.SAFE}),
    ...
    if hasattr(GimbalArbiter, "VALID_TRANSITIONS"):
        vt = GimbalArbiter.VALID_TRANSITIONS  # type: ignore[attr-defined]
