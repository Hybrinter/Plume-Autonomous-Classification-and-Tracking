"""Unit tests for flight.payload.tracking.tracker -- compute_iou() and match_blobs().
...
# module under test
from flight.payload.tracking import compute_iou, match_blobs
...
def test_new_blob_gets_new_id() -> None:
    ...
    assert result[0].blob_id != 5, (
        "Unmatched blob incorrectly inherited ID=5 from the previous blob"
    )
