...
from tools.ml_models.train.samples import _row_image, load_processed_pack
...
def write_manifest(path: str, manifest: Manifest) -> None:
    ...
    if manifest.ingest_path:
        payload["ingest_path"] = manifest.ingest_path
    if manifest.radiometry:
        payload["radiometry"] = manifest.radiometry
    dest = Path(path)
...
def _run_dir_containing(path: Path) -> Path | None:
    """Return the nearest parent run directory, if one exists."""
    for parent in path.parents:
        if (parent / "config.toml").is_file() or (parent / "summary.json").is_file():
            return parent
    return None


def resolve_export_hw(
    *,
    promotable: bool,
    override_spatial: bool,
    config_height: int,
    config_width: int,
...
def _as_int(value: object, default: int) -> int:
    """Return ``value`` when it is an int, otherwise ``default``."""
...
def _string_field(payload: dict[str, object], key: str) -> str:
    """Return a string checkpoint field, or an empty string."""
...
def export(config: ExportConfig) -> tuple[Path, Path, Manifest]:
    ...
    payload = _load_state(config.checkpoint_path)
    ...
    height, width = resolve_export_hw(
        ...
        config_height=int(config.input_height_px),
    ...
    if not isinstance(state, dict):
        raise ValueError(f"checkpoint state_dict is not a dict: {config.checkpoint_path}")
    ...
    manifest = Manifest(
        ...
        sha256=digest,
        quantization="fp32",
        ingest_path=_string_field(payload, "ingest_path"),
        radiometry=_string_field(payload, "radiometry"),
    )
...
def reexport_spatial(
    ...
    *,
    ...
    manifest = Manifest(
        ...
        sha256=digest,
        quantization="fp32",
        ingest_path=src_manifest.ingest_path if src_manifest is not None else "",
        radiometry=src_manifest.radiometry if src_manifest is not None else "",
    )
    manifest_path = dest.with_suffix(".json")
    write_manifest(str(manifest_path), manifest)
    return dest, manifest_path, manifest
