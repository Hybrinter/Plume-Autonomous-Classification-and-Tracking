...
def test_build_frames_count_and_shape() -> None:
    """build_frames returns N frames each a (2048, 2448) uint16 mosaic plane."""
    frames = build_frames(3)
    assert len(frames) == 3
    mosaic = np.asarray(frames[0].mosaic)
    assert mosaic.shape == (2048, 2448)
...
def test_build_frames_renders_uint16_mosaic() -> None:
    """Rendered frames are 2048x2448 uint16 mosaics within 12-bit range."""
...
def test_plume_brightens_nir_at_center() -> None:
    ...
    assert float(nir[880:920, 592:632].mean()) > float(nir[:40, :40].mean())
