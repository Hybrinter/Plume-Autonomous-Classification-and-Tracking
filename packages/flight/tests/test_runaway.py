...
def _pos(position: float, t: float) -> GimbalAxisState:
    return GimbalAxisState(position_deg=position, sample_timestamp_s=t)


def test_matching_motion_resets_strikes() -> None:
    """Encoder motion matching the commanded rate keeps the strike count at zero."""
    state = RunawayState(last_pos=_pos(0.0, 0.0), strike_count=2)
    new_state, fault = check_runaway(
        state, _pos(2.0, 1.0), 2.0, True, tolerance_deg_per_s=1.0, strike_limit=3
    )
...
def test_divergence_accumulates_strikes_then_faults() -> None:
    ...
    state = RunawayState(last_pos=_pos(0.0, 0.0), strike_count=0)
    for i in range(1, 3):
        state, fault = check_runaway(
            state,
            _pos(0.0, float(i)),  # gimbal not moving
            2.0,  # but commanded 2 deg/s elevation
            True,
    ...
    state, fault = check_runaway(
        state, _pos(0.0, 3.0), 2.0, True, tolerance_deg_per_s=1.0, strike_limit=3
    )
...
def test_non_unit_dt_locks_rate_units() -> None:
    ...
    matched_state, fault = check_runaway(
        state, _pos(4.0, 2.0), 2.0, True, tolerance_deg_per_s=0.5, strike_limit=3
    )
    assert fault is None
    ...
    diverged_state, fault = check_runaway(
        state, _pos(4.0, 2.0), 4.0, True, tolerance_deg_per_s=0.5, strike_limit=3
    )
    assert fault is None
...
def test_elevation_axis_divergence_strikes() -> None:
    ...
    new_state, fault = check_runaway(
        state,
        _pos(0.0, 1.0),  # encoder static
        2.0,  # but commanded 2 deg/s
        True,
        tolerance_deg_per_s=1.0,
        strike_limit=3,
    )
...
def test_no_rate_mode_or_missing_data_resets() -> None:
    ...
    state = RunawayState(last_pos=_pos(0.0, 0.0), strike_count=2)
    new_state, fault = check_runaway(
        state, _pos(0.0, 1.0), 2.0, False, tolerance_deg_per_s=1.0, strike_limit=3
    )
    assert fault is None
    assert new_state.strike_count == 0
    new_state, fault = check_runaway(
        new_state, None, 2.0, True, tolerance_deg_per_s=1.0, strike_limit=3
    )
    assert fault is None
