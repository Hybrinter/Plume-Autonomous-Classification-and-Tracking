...
def test_list_compare_and_rank_include_the_run(tmp_path: Path) -> None:
    """List, compare, and rank tables include the run id."""
    dest = _write_run(tmp_path, "cli-run", "classifier", 0.9)
    runs = discover_runs(tmp_path)
    assert "cli-run" in format_list(runs)
    assert "cli-run" in format_compare((dest,))
    assert "cli-run" in format_rank(rank_runs(runs, "mean_iou"))
