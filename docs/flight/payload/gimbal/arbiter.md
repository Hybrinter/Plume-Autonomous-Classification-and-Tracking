...
`GimbalArbiter` selects TRACKING, REWIND, FAST_REWIND, or SAFE. It does not emit
axis rates or torque. SAFE latches until ground clears it.

## Public interface

| Name | Kind | Description |
| --- | --- | --- |
| `ArbiterState` | dataclass | Immutable FSM snapshot: mode, blobs, aggregate liveness, observation age, miss count, hunt entry time |
...
timestamp_utc="", rewind_sharp_max_s=2.0)` returns
...
4. From TRACKING: empty packets increment the release counter. The first of
   ...
   with `r = 0`.
5. From REWIND: after `rewind_sharp_max_s` without a plume and not at the limb,
   promote to FAST_REWIND. Keep `rewind_entered_s`.
6. From REWIND or FAST_REWIND: an accepted aggregate returns to TRACKING and
   clears `rewind_entered_s`. Arrival at the limb with no plume also returns to
   TRACKING.
7. Outer coast ticks pass `vision_updated=False` and leave `miss_count` unchanged,
...
`GimbalConfig` science-limb and stow elevation. `rewind_sharp_max_s` is injected
from `OuterLoopConfig`.
