...
def _manifest_hw(manifest: Manifest) -> tuple[int, int]:
    """Return concrete height and width from a manifest input shape.

    Raises:
        ValueError: If the input shape is not a concrete NCHW tuple.
    """
    shape = manifest.input_shape
    if len(shape) < 4:
        raise ValueError(f"manifest input shape is not NCHW: {shape}")
    height = shape[2]
    width = shape[3]
    if height is None or width is None:
        raise ValueError(f"manifest input shape is not concrete: {shape}")
    return height, width
...
def _accept(
    ...
    *,
    ...
    When ``flight`` is true, expected shapes come from ``InferenceConfig``.
    When ``flight`` is false, expected shapes come from the manifest.
    """
    manifest = load_manifest(str(artifact.with_suffix(".json")))
    expected_input: tuple[int | None, ...]
    if flight:
        spec = InferenceConfig()
        height = spec.input_height_px
        width = spec.input_width_px
        expected_input = (1, len(spec.input_bands), height, width)
    else:
        height, width = _manifest_hw(manifest)
        expected_input = manifest.input_shape
    return accept_kind(
        kind,
        str(artifact),
        manifest,
...
def finalize(
    ...
    *,
    ...
    Args:
        ...
        flight: When true, the gate expects ``InferenceConfig`` shapes.
            When false, the gate expects the exported manifest shapes.
